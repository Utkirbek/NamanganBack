...
const searchProduct = async (req, res) => {
  try {
    ...
    if (search) {
      if(search.charAt(0) === "+"){
        products = await Product.find({code : search.slice(1)}).lean().populate("currency")
      }else{
        products = await Product.find( { title: { $regex: new RegExp(search, 'i') } }).lean().populate("currency")
      }
